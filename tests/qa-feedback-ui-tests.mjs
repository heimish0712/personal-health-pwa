...
export async function runQaFeedbackUiTests({ cdp, pollEvaluate, runtimeTest, profileDirectory, root }) {
  ...
  const shot = await cdp.send('Page.captureScreenshot', { format: 'png' }); fs.writeFileSync(path.join(root, 'tests/results/v0.7.0-calendar.png'), Buffer.from(shot.data, 'base64'));
