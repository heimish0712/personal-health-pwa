...
export async function runDietTests(test) {
  ...
  try {
    ...
    const oldDoc=structuredClone(globalThis.__BACKUP_TEST_DOCUMENT__);oldDoc.source.appVersion='0.6.0';oldDoc.source.dbVersion=1;const v1=await make('v1');const oldPreview=await v1.backupImportService.inspectFile(new Blob([canonicalJson(oldDoc)]));await v1.backupImportService.restorePreview(oldPreview.id);
    await test('DIET-BACKUP-07-V1',async()=>equal((await v1.backupSnapshotReader.readCurrentProfile()).data,oldDoc.data)&&(await binaries(v1)).length===0,'Original v1 JSON still restores exactly into DB2.');
