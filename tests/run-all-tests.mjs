...
const suitesToRun = [
  { script: 'tests/smoke-test.mjs', result: 'tests/results/v0.6.0-smoke.json', suite: 'smoke' },
  { script: 'tests/architecture-test.mjs', result: 'tests/results/v0.6.0-architecture.json', suite: 'architecture' },
  { script: 'tests/schema-test.mjs', result: 'tests/results/v0.6.0-schema.json', suite: 'schema' },
  { script: 'tests/exercise-service-test.mjs', result: 'tests/results/v0.6.0-exercise-service.json', suite: 'exercise-service' },
  { script: 'tests/backup-test.mjs', result: 'tests/results/v0.6.0-backup.json', suite: 'backup' },
  { script: 'tests/browser-runner.mjs', result: 'tests/results/v0.6.0-browser.json', suite: 'browser-runtime' }
];

for (const item of suitesToRun) {
  fs.rmSync(path.join(root, item.result), { force: true });
}
fs.rmSync(path.join(root, 'tests/results/v0.6.0.json'), { force: true });
...
const manualCases = [
  ...
  { id: 'APP-001', status: 'NOT_RUN', evidence: 'Deploy v0.6.0 to the user GitHub Pages repository.' },
  ...
  { id: 'CACHE-RUNTIME-001', status: 'NOT_RUN', evidence: 'Verify v0.4.0 App Shell replacement by v0.6.0 on the deployed origin.' }
];
...
const aggregatePath = path.join(root, 'tests/results/v0.6.0.json');
