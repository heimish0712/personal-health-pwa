...
v0.10은 설정에서 운동 예약을 Google 기본 캘린더로 단방향 반영한다. **실제 공개 Web Client ID는 아직 미설정**이며 config 또는 설정 입력으로 제공해야 연결된다. 기본 OFF, 로컬 기록/오프라인 기능은 그대로 사용한다. [설정·설계](docs/GOOGLE_CALENDAR.md), [실계정 QA](MANUAL_QA.md)를 확인한다. 실제 Google/설치형 PWA QA는 NOT RUN이다.
...
v0.5.0에서 **운동 → 이용권 관리**, **캘린더 → 예약**을 사용할 수 있습니다. v0.6.0에서 **체중 → 일반 체중/인바디/변화 그래프**와 캘린더·홈 측정 요약을 사용할 수 있습니다. v0.7.0에서 **식단·사진과 ZIP Backup v2**를 사용할 수 있습니다. v0.8.0에서 **홈 대시보드·통합 캘린더·선택 날짜 빠른 기록**을 사용할 수 있습니다. v0.9 운영 안정화와 v0.10 Google 연동을 추가했다. 후속 Supabase Sync는 [개발 로드맵](docs/ROADMAP.md)을 따릅니다.

## Backup Core v0.4.0

설정에서 **백업 내보내기 / 백업 복원**을 사용할 수 있습니다. 현재 Profile의 portable 13 Store를 JSON v1(사진 포함 시 ZIP v2)으로 저장하며 UUID·revision·시각·삭제 상태·과거 Template을 보존합니다. 파일은 암호화되지 않습니다.
