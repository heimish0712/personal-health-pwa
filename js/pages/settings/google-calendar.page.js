...
export async function mountGoogleCalendar(root, { services, isCurrent }) {
  ...
  section.innerHTML = `<h2>Google Calendar</h2><p>로컬 운동 예약을 선택한 Google 계정의 기본 캘린더에 단방향으로 전송합니다. 메모는 전송하지 않습니다. Google에서 수정해도 앱으로 가져오지 않습니다.</p>
    ...
    <label>Google Web Client ID (공개 설정)<input id="google-client-id" type="text" value="${e(globalThis.APP_CONFIG.GOOGLE_CLIENT_ID ?? '')}" autocomplete="off" placeholder="…apps.googleusercontent.com"></label>
    <p>Client ID는 인증 비밀이 아닙니다. 이 입력은 현재 화면에서만 유지됩니다. 매번 입력하지 않으려면 배포 설정에 등록하세요.</p>
    <button class="button button-secondary" id="google-prepare">1. Google 연결 준비</button>
    ...
    <button class="button button-secondary" id="google-status-refresh">상태 새로고침</button>
