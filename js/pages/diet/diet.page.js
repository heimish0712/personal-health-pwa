...
export async function renderDietDetail(c,id) {
  ...
  action(c,'#diet-back',()=>c.navigate('/diet'));action(c,'#diet-edit',()=>c.navigate(`/diet/log/${id}/edit`));
...
export async function renderDietForm(c,id) {
  ...
  form.addEventListener('input',()=>{dirty=true;});setNavigationGuard(()=>!busy&&!processing&&(!dirty||confirm('저장하지 않은 내용을 버리고 이동할까요?')));action(c,'#diet-cancel',()=>c.navigate('/diet'));
  ...
  form.addEventListener('submit',async(event)=>{event.preventDefault();if(busy||processing)return;busy=true;form.querySelector('#diet-save').disabled=true;errorNode.textContent='저장 중…';
    try{await refreshStorage();const value=(key)=>form.querySelector(`#diet-${key}`).value;const saved=await c.services.diet.save({eaten_at_local:value('at'),meal_type:value('meal'),content:value('content'),memo:value('memo')},id,row?.revision,kept.map((p)=>p.id),added);if(!c.isCurrent())return;await refreshStorage().catch(()=>{});dirty=false;busy=false;clearNavigationGuard();c.showToast('식단을 저장했습니다.');c.navigate(`/diet/log/${saved.id}`);}catch(error){if(c.isCurrent())errorNode.textContent=getActionErrorMessage(error,'식단을 저장하지 못했습니다. 기존 데이터는 유지됩니다.');}finally{busy=false;if(form.isConnected)form.querySelector('#diet-save').disabled=false;}
